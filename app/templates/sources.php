<div class="container">
	...
	<div class="boxed">
		...
		<ul>
			<li><strong>NAS</strong><br><span class="help-block">where all network mounts fit</span></li>
			<li><strong>USB</strong><br><span class="help-block">the content of the USB drive, if plugged</span></li>
	...
	<form id="mount-list" class="button-list" method="post">
		...
		<p><a href="/sources/edit/<?=$mount['id'] ?>" class='btn btn-lg btn-default btn-block'> <i class='fa <?php if ($mount['status'] == 1): ?> fa-check green <?php else: ?> fa-times red <?php endif ?> sx'></i> NAS/<?=$mount['name'] ?>&nbsp;&nbsp;&nbsp;&nbsp;<span>//<?=$mount['address'] ?>/<?=$mount['remotedir'] ?></span></a></p>
		<?php endforeach ?>
